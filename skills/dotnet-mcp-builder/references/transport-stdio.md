...
- You need server-to-client features (elicitation, and the spec-deprecated sampling/roots) — STDIO always supports them, no `Stateless` flag to worry about.
...
The SDK sends `serverInfo` (name + version) during connection negotiation — in the `server/discover` response on spec 2026-07-28, or the legacy `initialize` response for older clients; both are handled automatically. By default it derives them from your assembly. To override:
