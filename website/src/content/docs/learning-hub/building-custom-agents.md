...
**include-custom-instructions** *(v1.0.86+)*: Set to `true` to have the agent also pick up repository instruction files (`AGENTS.md`, `copilot-instructions.md`, `CLAUDE.md`) in addition to its own frontmatter and body. This is useful when a custom agent should still follow your team's general coding standards rather than operating in complete isolation:
