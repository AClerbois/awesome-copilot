...
*(v1.0.85+)* Set `transcriptView` to `"concise"` in your settings to group tool activity into expandable work summaries instead of showing every tool call inline — useful for keeping long agent sessions scannable. The same release added `/settings` options to opt in to **context management tools** for agents and subagents, giving the model explicit tools for managing its own context window during long-running work.

### Autopilot Stops After Task Completion

*(v1.0.86+)* Autopilot now stops once a task is accepted as complete, instead of continuing to work unexpectedly afterward. This makes `--autopilot` and `/autopilot` sessions easier to predict when scripting or running unattended.

### Agents Can Opt Into Repository Instructions

*(v1.0.86+)* Custom agents can opt into repository instruction files (`AGENTS.md`, `copilot-instructions.md`, `CLAUDE.md`) by setting `include-custom-instructions: true` in their frontmatter, so the agent's own persona is layered on top of your team's existing conventions instead of replacing them. See [Building Custom Agents](../building-custom-agents/) for the frontmatter syntax.
