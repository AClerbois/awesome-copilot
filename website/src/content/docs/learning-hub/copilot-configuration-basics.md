...
**Auto routing tier startup defaults** *(v1.0.87+)*: Organizations can set user and managed startup defaults for the Auto routing tier, including a strict policy (users cannot change it) or a user-overridable policy (Auto is the default but individuals can still pick a specific model). Check with your organization admin if you expect a different default model than what a new session starts with.
...
**Fork during an active turn** *(v1.0.88+)*: `/fork` no longer requires waiting for the current turn to finish — run it while Copilot is still working, and the new session branches off immediately.
